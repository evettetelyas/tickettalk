...
class Offer < ApplicationRecord
  ...
  def as_json(options)
    super(options).merge(message: "You have a new #{self.status} offer from #{User.find(self.offer_user_id).username}", confirm_message: "You have a new #{self.status} offer from #{User.find(self.user_id).username}")
  end
